...
class TestInterpreter(unittest.TestCase):
    ...
    hello_worlds = list(
        map(lambda f: "test/assets/hello_world." + f, ["brainfuck", "brainloller.png", "braincopter.png"]))
    ...
    def test_conversion(self):
        for source, output in itertools.product(self.hello_worlds, ["brainfuck", "brainloller", "braincopter"]):
            with self.subTest(source=source, output=output):
                ...
                # convert
                args = main.parser_main.parse_args(
                    ["convert", "-o", output, source, tmp.name, "-i", "test/assets/earth.png"])
                # print(args)
